...
internal/
  cli/                    → cobra command definitions (one file per subcommand)
    ...
    setproxy.go           → ffm set-proxy: configures socketio_port / use_ssl / per-site host_name inside the container for reverse-proxy deployments (dev benches only; blocked for prod); --reset restores direct-access defaults; --print-caddy / --print-nginx emit ready-to-paste config snippets
...
- **Mode-aware commands**: `shell.go` uses zsh for dev, bash for prod. `start.go` skips skills install + bench start for prod (compose `command:` handles services). `restart.go` passes `Mode` to `bench.ComposeData` when rebuilding. `setproxy.go` returns an error for prod benches. `list.go` shows MODE column. `status.go` shows domain URL for prod.
...
- **Prod with existing Caddy/Nginx on 80/443**: Use `--no-ssl`. `EnsureNetwork()` is called instead of `EnsureHTTPS()`, so Traefik does not try to bind port 443. The compose exposes `WebPort` and `SocketIOPort` directly on the host. The user points Caddy at those ports. After create, `socketio_port` defaults to 80; if Caddy handles HTTPS the user must manually run `bench set-config -gp socketio_port 443 && bench set-config -gp use_ssl 1` inside the container. **Known limitation:** `ffm set-proxy` is blocked for prod benches — there is no ffm command that applies this correction automatically.
