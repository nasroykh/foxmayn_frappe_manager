...
internal/
  cli/                    → cobra command definitions (one file per subcommand)
    ...
    create.go             → multi-step bench provisioning pipeline (port alloc → compose+Dockerfile write → docker build → bench init via docker compose run → docker up → site creation → app install → mode-specific steps); supports --mode dev (default) and --mode prod; supports --db-type mariadb (default) and --db-type postgres; interactive form (runCreateFormFull) asks mode first, then DB engine, then shows dev or prod follow-up fields; automatic rollback on failure
